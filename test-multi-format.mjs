...
/**
 * Test script for PDFSpark file upload and conversion to multiple formats
...
// Format to test (optional command line argument)
const TARGET_FORMAT = process.argv[3] || 'all';  // 'all', 'docx', 'txt', 'jpg', 'png'
...
async function pollStatus(operationId, maxAttempts = 20) {
...
// Run test for one format
async function testFormat(fileId, format) {
  console.log(`\n=== Testing conversion to ${format} ===`);
  
  // Step 1: Start conversion
  const conversionResult = await convertFile(fileId, format);
  if (!conversionResult) return false;
  
  // Step 2: Poll for status
  const finalStatus = await pollStatus(conversionResult.operationId);
  if (!finalStatus || finalStatus.status !== 'completed') return false;
  
  // Step 3: Get download URL
  const downloadResult = await getDownloadUrl(conversionResult.operationId);
  if (downloadResult && downloadResult.downloadUrl) {
    console.log(`\n✅ Conversion to ${format} completed successfully!`);
...
async function runMultiFormatTest() {
  console.log(`\n=== Testing PDFSpark multi-format conversion at ${API_URL} ===\n`);
  ...
  // Step 3: Test conversions for different formats
  const formats = TARGET_FORMAT === 'all' 
    ? ['docx', 'txt', 'jpg', 'png'] 
    : [TARGET_FORMAT];
  ...
  for (const format of formats) {
    results[format] = await testFormat(uploadResult.fileId, format);
  }
  
  // Step 4: Display summary
  console.log('\n=== Conversion Test Summary ===');
  for (const [format, success] of Object.entries(results)) {
    console.log(`${format}: ${success ? '✅ Success' : '❌ Failed'}`);
  }
}
